        ...
        if(SUCCEEDED(CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&link)))) {
            ...
            if(SUCCEEDED(link->QueryInterface(IID_PPV_ARGS(&pf)))) {
                ...
                if(SUCCEEDED(link->QueryInterface(IID_PPV_ARGS(&store)))) {
                    ...
                    PropVariantClear(&pv);
                    if(SUCCEEDED(store->GetValue(PKEY_Company, &pv)) && pv.vt==VT_LPWSTR) company = CW2A(pv.pwszVal);
                    PropVariantClear(&pv);
                    if(SUCCEEDED(store->GetValue(PKEY_Category, &pv)) && pv.vt==VT_LPWSTR) machine = CW2A(pv.pwszVal);
                    PropVariantClear(&pv);
                    if(SUCCEEDED(store->GetValue(PKEY_Calendar_Location, &pv)) && pv.vt==VT_LPWSTR) location = CW2A(pv.pwszVal);
                    PropVariantClear(&pv);
                    if(SUCCEEDED(store->GetValue(PKEY_StartDate, &pv)) && pv.vt==VT_FILETIME) start = std::chrono::system_clock::from_time_t(FileTimeToUnixTime(pv.filetime));
                    PropVariantClear(&pv);
                    if(SUCCEEDED(store->GetValue(PKEY_EndDate, &pv)) && pv.vt==VT_FILETIME) end = std::chrono::system_clock::from_time_t(FileTimeToUnixTime(pv.filetime));
                    PropVariantClear(&pv);
                    store->Release();
                }
                pf->Release();
...
void Trip::ApplyToShortcut(const std::filesystem::path& shortcutPath) const {
    ...
    if(SUCCEEDED(link->QueryInterface(IID_PPV_ARGS(&store)))) {
        ...
        store->SetValue(PKEY_Title, pv); PropVariantClear(&pv);
        // human friendly description
        InitPropVariantFromString(CA2W(title_.c_str()), &pv);
        store->SetValue(PKEY_FileDescription, pv); PropVariantClear(&pv);
        InitPropVariantFromString(CA2W(company_.c_str()), &pv);
        store->SetValue(PKEY_Company, pv); PropVariantClear(&pv);
        InitPropVariantFromString(CA2W(machine_.c_str()), &pv);
        store->SetValue(PKEY_Category, pv); PropVariantClear(&pv);
        InitPropVariantFromString(CA2W(location_.c_str()), &pv);
        store->SetValue(PKEY_Calendar_Location, pv); PropVariantClear(&pv);
        ...
        InitPropVariantFromFileTime(&ftStart, &pv); store->SetValue(PKEY_StartDate, pv); PropVariantClear(&pv);
        InitPropVariantFromFileTime(&ftEnd, &pv); store->SetValue(PKEY_EndDate, pv); PropVariantClear(&pv);
        wchar_t dur[32]; swprintf(dur, 32, L"%d Tage", GetDuration());
        InitPropVariantFromString(dur, &pv); store->SetValue(PKEY_Duration, pv); PropVariantClear(&pv);
        store->Commit();
        store->Release();
    }
    IPersistFile* pf = nullptr;
